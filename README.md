...
매장 방문 손님이 질문 5개에 답하면 취향에 맞는 맥주 한 종을 추천해 주는
...
현재 맥주 7종 × 스타일 7종, 질문 5개(그중 3개가 추천에 반영)로 동작한다.
스타일 질문은 종류(질문 4) · 맛(질문 5) 두 화면으로 나뉘어 있고, 추천을 정하는 건
맛을 묻는 질문 5다.
...
- 랜딩의 "질문 5개에 답하면 …" 문구와 `질문 0 / 5` 표시
- 뒤로가기 · 다음 단계 이동 로직
- 결과 화면으로 넘어가는 시점

즉 **질문 개수가 코드 어디에도 박혀 있지 않다.** 질문을 6개로 늘려도 고칠 파일은
`questions.ts` 뿐이다.
