 ...
 * 질문 한 개를 보여주는 화면.
 *
 * 제목 · 선택지는 전부 현재 질문 데이터에서 읽는다. 스텝퍼와 "STEP n/N" 의 N 도
 * `totalSteps` 에서 오므로 질문이 늘어도 이 파일은 그대로다.
 */
export function QuizScreen() {
  const { question, step, totalSteps, answers, select, back } = useQuiz();
        ...
        <div className="flex flex-col gap-3">
          {question.options.map((option, index) => (
            <OptionCard
              ...
              option={option}
              index={index}
              selected={option.id === answers[question.key]}
