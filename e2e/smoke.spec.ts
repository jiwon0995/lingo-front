 ...
 * (맥주 이름 · 질문 개수)은 값을 박지 않고 화면에서 읽어 비교한다.
 */
...
async function chooseOption(page: Page, index: number) {
  ...
  // 260ms 뒤 다음 스텝으로 넘어간다 — 제목이 바뀌면 전환된 것
  await expect(page.locator("h2")).not.toHaveText(title ?? "", {
    timeout: 5_000,
  });
}

test("랜딩 → 퀴즈 4문항 → 결과 → 다이스 리롤", async ({ page }) => {
  await page.goto("/");

  /* ---------- 랜딩 ---------- */
  await expect(page.getByRole("heading", { name: "나에게 맞는 맥주 찾기" })).toBeVisible();
  await expect(page.getByText("간단한 질문 4개에 답하면 딱 맞는 맥주를 추천해드려요.")).toBeVisible();
  await expect(page.getByText("질문 0 / 4 · 30초면 충분해요")).toBeVisible();
